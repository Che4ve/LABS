...
sparseMatrix* initSparseMatrix(void)
{
    ...
    matrix->vector = calloc( MAX_VECTOR_SIZE, sizeof(vectorElement) );
    matrix->rows = 0;
    matrix->cols = 0;
    matrix->vectorSize = 0;
    return matrix;
...
int readSparseMatrix(sparseMatrix* matrix, FILE* stream)
{
    ...
    for (int i = 1; i <= rows; i++) {
        for (int j = 1; j <= cols; j++) {
            ...
            pushBack(matrix, i, j, value);
        }
    }
    matrix->vector[matrix->vectorSize - 1] = NilElem;
    matrix->vector = realloc(matrix->vector, matrix->vectorSize * sizeof(vectorElement));
    return 1;
}
